...
class NewsController extends BaseController
{
    ...
    public function details($id,Request $request){
        ...
        if(count($news['image_path'])){
            ...
            $news['image_path'] = 'http://10.16.10.182:81'.($news['image_path'][0]); //TODO
        }else{
            $news['image_path'] = '';
        }
        ...
        $user_id = !empty(session('user_id')) ? session('user_id') : $userId;
    ...
    public function get_collect_news(Request $request){
        ...
        if($params['user_id'] == -1){
            //根据token获取用户信息
            $url = 'http://10.16.10.182:9051/auth/login/store/token?token=' . $token;
            $ch = curl_init();
        ...
        $arr = array();
        foreach($list as $key => $value){
            $arr[$key] = News::select('id','title','image','introduce','gmt_create')
            ...
            if(count($arr[$key]) == 0){
                ...
                $arr[$key]['url'] = 'http://10.16.10.182:81/home/news/details/' . $value['news_id'] .'?from=collect&userId='.$value['user_id'];
                //图片
                $arr[$key]['image_path'] = array_values(FileController::getFilePath($arr[$key]['image']));
                if(count($arr[$key]['image_path'])){
                    $arr[$key]['image_path'] = 'http://10.16.10.182:81' . $arr[$key]['image_path'][0];
                }else{
                    $arr[$key]['image_path'] = '';
                }
            }
